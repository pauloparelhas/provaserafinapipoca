# Auditoria de Cobertura — Ciências 2º tri (Properties of Matter)
**Data:** 01/07/2026 · **Prova aplicada:** 25/06 (nota 9.3/10) · **Método:** matriz insumo→produto + prova-espelho.
**Escopo desta auditoria:** confirmar buracos que impediriam gabaritar estudando SÓ pelo nosso material.

## 1. Matriz de rastreabilidade INSUMO → PRODUTO

Legenda: **TREINA** (criança age sobre o átomo) · **MENCIONA** (só texto passivo) · **AUSENTE**.

| Átomo (origem no insumo) | Eixo | Excl/Compart | Status | Onde |
|---|---|---|---|---|
| Nomear solid/liquid/gas | estados | — | ✅ TREINA | simulado, dragdrop, popit |
| Solid: keeps its shape | forma | excl. solid | ✅ TREINA | simulado |
| Liquid: takes shape of container | forma | excl. liquid | ✅ TREINA | simulado (Q "takes the shape") |
| Liquid: flows / can be poured | forma | excl. liquid | ✅ TREINA | simulado, dragdrop |
| Solid: does not flow | forma | excl. solid | ✅ TREINA | simulado |
| Gas: changes shape / no fixed shape | forma | excl. gas | ✅ TREINA | simulado, resumo |
| **take up space (ocupa espaço)** | volume | **COMPART. solid+liquid+gas** | 🟡 **MENCIONA** | só mindmap+resumo ("all three…"), nunca exercitado |
| **have mass / weight** | massa | **COMPART. solid+liquid+gas** | 🟡 **MENCIONA** | só mindmap+resumo |
| Particle arrangement (packed/slide/far) | partículas | por estado | ✅ TREINA | simulado (ordenar), dragdrop, popit |
| Examples reais (ice/pencil/water/steam/cloud…) | exemplos | por estado | ✅ TREINA | dragdrop (sort), galeria |
...
- 🔴🟡 **#1 — Propriedade COMPARTILHADA nunca treinada (a maior).** "take up space" e "have mass"
  estão nas listas de CADA estado no slide de propriedades da escola. Os produtos ou só mencionam
  (resumo/mindmap) ou treinam cada propriedade como pertencente a UM estado (`col:'liquid'` no
  dragdrop; opções de estado único no simulado). Nunca houve o formato **"qual(is) estado(s) tem
...
- 🟠 **#4 — Peso mal alocado:** absorb/repel está nos **7** produtos e **não caiu**; viscosidade e
  propriedade-compartilhada (as duas maiores questões) em zero produtos de prática.
- 🟠 **#5 — "Uso cotidiano" de cada estado** (Q2) treinado fraco, sobretudo GÁS ("I can cloud" saiu
  mal). Temos exemplos, não o framing de USO.

## 3. Prova-espelho × prova real (9 questões)

| Q | O que cobrou | Formato | Nosso treino |
|---|---|---|---|
| 1 | Nomear os 3 estados | escrever | ✅ |
| 2 | 1 uso cotidiano de cada estado + desenho | produzir | 🟠 parcial (gás fraco) |
