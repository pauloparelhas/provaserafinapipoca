...
A.forEach(function(it){
 var ok=it.opts.filter(function(o){return o.ok===1;});
 var nos=it.opts.filter(function(o){return typeof o.no==="string" && o.no.length>0;});
 console.log(it.id+" | "+it.eixo+" | opts="+it.opts.length+" | ok="+ok.length+" | no="+nos.length+" | acende="+(it.acende?JSON.stringify(it.acende):"-")+" | texto="+(it.texto?it.texto.length:0)+" | origem="+(it.origem==="Treino no estilo da prova")+" | gab="+(ok[0]?ok[0].t:"???"));
});
